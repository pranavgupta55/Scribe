# Building Framework

> Deploying front-end to Vercel requires creating a GitHub repo and setting up a Vercel project

## How-To / Method
- Front-end deployment to Vercel necessitates creating a GitHub repository to host code and assets
- A Vercel project must be initialized with specific configuration files to enable deployment

## Claims
- Front-end deployment to Vercel requires GitHub repository creation and Vercel project setup — [start_a_1_person_business_with_claude_4_hour_course_2026.txt § "Deploying Automations to Production"]

## Connections
- Both topics agree that setting up GitHub for deployment is essential for front-end deployment to Vercel. _(connects to: GitHub Integration)_

---
_Topic appears in 1 source(s) · 1 claim(s) · 1 connection(s)_
_Sources: start_a_1_person_business_with_claude_4_hour_course_2026.txt_
