# Webinar Strategy

> Webinar pitch: soft offer for free 'savings assessment' showing what client saves on payroll

## Claims
- 10X move: run webinars targeting OT/PT/SLP owners with 5+ locations using Meta and Andromeda ads — [stop_building_10_businesses_build_one_to_10m_sRBO1pxSqK4.txt]
- Ad creative: show 10 use cases of painful tasks a VA could handle; avoid boring 'here's how we do it' pitch — [stop_building_10_businesses_build_one_to_10m_sRBO1pxSqK4.txt]
- Webinar pitch: soft offer for free 'savings assessment' showing what client saves on payroll — [stop_building_10_businesses_build_one_to_10m_sRBO1pxSqK4.txt]
- Positioning: 'Better than what you have AND costs less' - customers want both, not just cheaper — [stop_building_10_businesses_build_one_to_10m_sRBO1pxSqK4.txt]

---
_Topic appears in 1 source(s) · 4 claim(s) · 0 connection(s)_
_Sources: stop_building_10_businesses_build_one_to_10m_sRBO1pxSqK4.txt_
